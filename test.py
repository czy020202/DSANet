...
def main():
    ...
    if args.mode == 'eval':
        ...
        if initial_checkpoint:
            ...
            checkpoint['state_dict'].pop('feature_net.resnet50.layer2.1.pe.pe')
            checkpoint['state_dict'].pop('feature_net.resnet50.layer2.4.pe.pe')
            checkpoint['state_dict'].pop('feature_net.resnet50.layer3.1.pe.pe')
            checkpoint['state_dict'].pop('feature_net.resnet50.layer3.4.pe.pe')
            checkpoint['state_dict'].pop('feature_net.resnet50.layer3.7.pe.pe')
...
def eval(net, dataset, save_dir=None):
    ...
    for i, (input, truth_bboxes, truth_labels) in tqdm(enumerate(dataset)):
        try:
    ...
    annotations_filename = '/ChestCT2025/test_anno.csv'
