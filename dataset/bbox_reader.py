    ...
    def __getitem__(self, idx):
        ...
        if self.mode in ['train', 'val']:
            if not is_random_img:
                ...
                imgs = np.load(os.path.join(self.data_dir, 'CT', '%s.npy' % (filename)))
        ...
        if self.mode in ['eval']:
            ...
            image = np.load(os.path.join(self.data_dir, 'CT',  '%s.npy' % (filename)))
    ...
    if do_flip:
        ...
        for i in range(3):
            if flipid[i]==-1:
                tem = [0, 2, 4]
                ax = tem[i]
                target[ax], target[ax+1] = np.array(sample.shape[i+1])-target[ax+1], np.array(sample.shape[i+1])-target[ax]
                bboxes[:,ax], bboxes[:, ax+1] = np.array(sample.shape[i+1])-bboxes[:,ax+1], np.array(sample.shape[i+1])-bboxes[:,ax]
