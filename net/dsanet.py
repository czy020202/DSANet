...
class CropRoi(nn.Module):
    ...
    def forward(self, f, inputs, proposals):
        ...
        for p in proposals:
            ...
            c0 = np.array([z_start.detach().cpu(), y_start.detach().cpu(), x_start.detach().cpu()])
            c1 = np.array([z_end.detach().cpu(), y_end.detach().cpu(), x_end.detach().cpu()])
