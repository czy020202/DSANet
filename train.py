from net.dsanet import DSANet
import time
from dataset.collate import train_collate, test_collate, eval_collate
from dataset.bbox_reader import BboxReader
from utils.util import Logger
from config_real import train_config, data_config, net_config, config
import pprint
...
def train(net, train_loader, optimizer, epoch, writer):
    ...
    rpn_stats = np.asarray(torch.tensor(rpn_stats).detach().cpu(), np.float32)
